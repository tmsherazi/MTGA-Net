...
# ----------------------------
# Branch Attention Module (BAM) - Core of MTGA-Net
# Implements Eqs. (1), (4)-(8) from the paper
# ----------------------------
class BranchAttentionModule(nn.Module):
    def __init__(self, reduction_ratio=16):
        super(BranchAttentionModule, self).__init__()
        C1, C2, C3 = 256, 512, 1024  # ResNet152 stage channels

        # DNN1: stage1 (256) → attention for stage2 (512)
        self.dnn1 = nn.Sequential(
            nn.Flatten(),
            nn.Linear(2 * C1, C1 // reduction_ratio),  # 2*C1 due to SPP+SFP concat
            nn.ReLU(inplace=True),
            nn.Linear(C1 // reduction_ratio, C2),
            nn.Softmax(dim=1)
        )

        # DNN2: stage2 + b1 → attention for stage3 (1024)
        self.dnn2 = nn.Sequential(
            nn.Flatten(),
            nn.Linear(2 * C2, C2 // reduction_ratio),
            nn.ReLU(inplace=True),
            nn.Linear(C2 // reduction_ratio, C3),
            nn.Softmax(dim=1)
        )

        # DNN3: stage3 + b2 → attention for stage4 (2048)
        self.dnn3 = nn.Sequential(
            nn.Flatten(),
            nn.Linear(2 * C3, C3 // reduction_ratio),
            nn.ReLU(inplace=True),
    ...
    def spatial_pooling(self, x):
        """SPP + SFP: Concatenate global avg + max pooling per channel (Eq. 1)"""
        avg_pool = F.adaptive_avg_pool2d(x, 1)  # (B, C, 1, 1)
        max_pool = F.adaptive_max_pool2d(x, 1)  # (B, C, 1, 1)
        return torch.cat([avg_pool, max_pool], dim=1)  # (B, 2C, 1, 1)

    def forward(self, x1, x2, x3, x4):
        # Eq. (4): b1 = DNN1(Concat[SPP(x1), SFP(x1)])
        g1 = self.spatial_pooling(x1)  # (B, 512, 1, 1)
        b1 = self.dnn1(g1.squeeze(-1).squeeze(-1)).unsqueeze(-1).unsqueeze(-1)  # (B, 512, 1, 1)
        x2_weighted = x2 * b1

        # Eq. (6): b2 = DNN2(Concat[SPP(x2), SFP(x2)] · b1)
        g2 = self.spatial_pooling(x2_weighted)  # (B, 1024, 1, 1)
        b2 = self.dnn2(g2.squeeze(-1).squeeze(-1)).unsqueeze(-1).unsqueeze(-1)  # (B, 1024, 1, 1)
        x3_weighted = x3 * b2

        # Eq. (7): b3 = DNN3(Concat[SPP(x3), SFP(x3)] · b2)
        g3 = self.spatial_pooling(x3_weighted)  # (B, 2048, 1, 1)
        b3 = self.dnn3(g3.squeeze(-1).squeeze(-1)).unsqueeze(-1).unsqueeze(-1)  # (B, 2048, 1, 1)
...
# ----------------------------
# Data Loading (Exact Paths from Your Setup)
# ----------------------------
